...
export default function Layout({ children }) {
  ...
  useEffect(() => {
    if(email){
      /*
      axios.get('/api/users?q='+JSON.stringify({email: email, data: true })).then(res => {
        const jsn = res.data;
        if(jsn.found){
    ...
    >
      <MenuItem>
        <IconButton aria-label="show 4 new mails" color="inherit">
          <Badge display={false} badgeContent={0} color="secondary">
            <MailIcon />
          </Badge>
        </IconButton>
        <p>Messages</p>
  ...
  return (
    <div className={classes.grow}>
      ...
      <Typography>&copy; Money App 2021 (from Samuel Nwabueze) </Typography>
      </footer>
    </div>
  );
}
